            ...
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <span
                        ...
                        class="day-summary mx-3 px-1 text-start"
                        style="flex-grow: 1"
                    ...
                    <div class="btn-toolbar" role="toolbar" aria-label="Toolbar with button groups">
                        ...
                        <div class="btn-group me-2" role="group" aria-label="Indicators group">
                            <button type="button" class="btn" @click="toggleNonWorkingStatus(!dayInfo.nonWorkingDay)">
                                <span v-if="!dayInfo.nonWorkingDay" :title="$t('calendar.actions.markHoliday')">
                                    🎉
                                </span>
                ...
                <div class="modal-body">
                    ...
                    <p v-if="dayInfo.vacation" class="mb-2 mt-2">🏖️ {{ $t('calendar.vacation') }}</p>
                    ...
                    <div v-if="!dayInfo.nonWorkingDay && !dayInfo.vacation" class="w-100 container">
                        <progress-bar
                            :big="true"
                            v-if="dayInfo.workingMinutes > 0"
                            :synchronized="durationOfSynced"
                            :loggedHereOnly="durationOfLoggedOnly"
                            :total="dayInfo.workingMinutes"
                    ...
                    <div v-if="sortedWorklogs.length == 0" style="height: 200px"></div>
                </div>
            </div>
        </div>
    </div>
    ...
    export default defineComponent({
        ...
        methods: {
            ...
            saveText() {
                ...
                api.updateDaySummary(this.dayInfo.date, this.text).then(response => {
            ...
            changeWorkingMinutes(direction) {
                ...
                if (newValue >= 0 && newValue <= 24 * 60) {
                    api.updateWorkingMinutesCount(this.dayInfo.date, newValue).then(response => {
            ...
            toggleNonWorkingStatus(newValue) {
                api.updateDayNonWorkingStatus(this.dayInfo.date, newValue).then(response => {
            ...
            toggleVacation(newValue) {
                api.updateDayVacation(this.dayInfo.date, newValue).then(response => {
