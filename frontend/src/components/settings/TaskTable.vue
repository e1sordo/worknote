            ...
            <tbody>
                <tr v-for="task in filteredTasks" :key="task.entityId">
                    ...
                    <td class="middle-vertical-children">
                        <span class="fs-3 pr-2">{{
                            taskTypeMeta[task.type].icon
                        }}</span>
                        <del v-if="task.closed" class="text-muted">{{
                            task.title
                        }}</del>
                        ...
                        <button
                            ...
                            @click="openCloseTask(task)"
                        >
                            <span v-if="task.closed">Open</span>
                            <span v-else>Close</span>
                        </button>
                    </td>
                    <td>{{ task.defaultValue }}</td>
...
<script lang="ts">
import TaskForm from "@/components/settings/TaskForm.vue";
    ...
    computed: {
        filteredTasks(): TaskWithUsage[] {
            return this.tasks.filter((task) =>
                (task.title + task.id + task.type).includes(this.filter)
            );
        },
    },
    methods: {
        async fetchTasks() {
            try {
                ...
                //     }
                // }
                const response = await api.activeTasks();
        ...
        upsertTask(taskForm: Task) {
            api.upsertTask(taskForm).then((response) => {
                ...
                console.log(
                    "Task was upserted. Response status: " + response.status
                );
                this.editedTask = {} as Task;

                const task = response.data;

                var foundIndex = this.tasks.findIndex(
                    (item) => item.entityId == task.entityId
                );
